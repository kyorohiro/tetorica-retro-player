...
export class TetoricaRetroAudioNode {
  ...
  updateAudioNodes() {
    ...
    if (lowpass && highshelf && drive) {
      ...
      drive.oversample = amount > 0.001 ? "4x" : "none";
      const driveAmount = amount * 0.6;
      if (Math.abs(driveAmount - this._driveCurveAmount) > 0.002) {
        try {
    ...
    if (roomDryGain && roomWetGain) {
      ...
      if (roomConvolver) {
        const shouldConnect = amount > 0.001;
        if (shouldConnect && !this._roomConvolverConnected) {
          ...
          const upstream = stereoWidth ?? this.nodes.tapeSaturator;
          try { upstream?.disconnect(roomConvolver); } catch { /* already disconnected */ }
          try { roomConvolver.disconnect(roomWetGain); } catch { /* already disconnected */ }
    ...
    if (hallReverbWetGain) {
      ...
      if (hallReverbConvolver && masterGain) {
        ...
        if (shouldConnect && !this._hallReverbConvolverConnected) {
          ...
          try { masterGain.disconnect(hallReverbConvolver); } catch { /* already disconnected */ }
          try { hallReverbConvolver.disconnect(hallReverbWetGain); } catch { /* already disconnected */ }
    ...
    if (tapeSaturator) {
      ...
      tapeSaturator.oversample = tapeSatAmount > 0.001 ? "4x" : "none";
      if (Math.abs(tapeSatAmount - this._tapeSatCurveAmount) > 0.002) {
        try {
