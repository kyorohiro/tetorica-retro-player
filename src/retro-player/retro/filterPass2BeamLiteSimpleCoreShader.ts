export const FILTER_FRAGMENT_PASS2_BEAM_LITE_SIMPLE_CORE = `#version 300 es
...
vec3 applyBeamCross(vec2 gridUv)
{
  ...
  for (int sy = -1; sy <= 1; sy++) {
    for (int sx = -2; sx <= 2; sx++) {
      ...
      vec3 rightSample = needsHorizontalNeighbors
        ? sampleEmitterColor(emitterCell + vec2(1.0, 0.0), sourceSize)
        : centerSample;
      vec3 sampleColor = applyBeamInputPostProcess(centerSample, leftSample, rightSample, emitterCell);
  ...
  vec3 whiteCore = vec3(1.0) * highlightMask * (0.065 + luminance * 0.17);
...
void main(void)
{
  ...
  finalColor = vec4(clamp(beamColor, 0.0, 1.0), beamLuma);
}
`;
