...
function getWideGlowUpdateInterval(filterState: RetroVideoFilterState | null) {
  if (!filterState) return 1;
  const value = filterState.wideGlowUpdateInterval;
  return value === 2 || value === 4 || value === 8 || value === 12 ? value : 1;
}
...
type CompositePrepUniformLocations = {
  uTexture: WebGLUniformLocation | null;
  uTargetSize: WebGLUniformLocation | null;
  uSamplingMode: WebGLUniformLocation | null;
};
...
type WideGlowDownsampleUniformLocations = {
  uTexture: WebGLUniformLocation | null;
  uTexelSize: WebGLUniformLocation | null;
};

type WideGlowBlurUniformLocations = {
  uTexture: WebGLUniformLocation | null;
  uTexelSize: WebGLUniformLocation | null;
...
type WideGlowCompositeUniformLocations = {
...
const shouldUsePass2SamplingPrep = (filterState: RetroVideoFilterState) =>
  !isNearestSamplingMode(filterState) &&
...
export class TetoricaRetroVideoPipeline {
  ...
  private compositePrepLocs: CompositePrepUniformLocations | null = null;
  ...
  private compositeApplyFbo: WebGLFramebuffer | null = null;
  private compositeApplyTexture: WebGLTexture | null = null;
  private compositeApplyFboWidth = 0;
  private compositeApplyFboHeight = 0;
  private compositeApplyTextureSamplingFilter: number | null = null;
  private pass2SamplingFbo: WebGLFramebuffer | null = null;
  private pass2SamplingTexture: WebGLTexture | null = null;
  private pass2SamplingFboWidth = 0;
  private pass2SamplingFboHeight = 0;
  private pass2SamplingTextureSamplingFilter: number | null = null;
  ...
  private finalSceneFbo: WebGLFramebuffer | null = null;
  private finalSceneTexture: WebGLTexture | null = null;
  private finalSceneFboWidth = 0;
  private finalSceneFboHeight = 0;
  private finalSceneTextureSamplingFilter: number | null = null;
  private wideGlowQuarterFbo: WebGLFramebuffer | null = null;
  private wideGlowQuarterTexture: WebGLTexture | null = null;
  private wideGlowQuarterFboWidth = 0;
  private wideGlowQuarterFboHeight = 0;
  private wideGlowQuarterTextureSamplingFilter: number | null = null;
  private wideGlowBlurFbo: WebGLFramebuffer | null = null;
  private wideGlowBlurTexture: WebGLTexture | null = null;
  private wideGlowBlurFboWidth = 0;
  private wideGlowBlurFboHeight = 0;
  private wideGlowBlurTextureSamplingFilter: number | null = null;
  ...
  private wideGlowCacheInitialized = false;
  private wideGlowCacheKey = "";
  ...
  private ensureCompositeApplyFbo(width: number, height: number) {
    if (
      this.compositeApplyFboWidth === width &&
      this.compositeApplyFboHeight === height &&
      this.compositeApplyFbo
    ) {
      return;
    }
    ...
    const tex = gl.createTexture();
    if (!tex) throw new Error("Failed to create composite apply FBO texture.");
    gl.bindTexture(gl.TEXTURE_2D, tex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
    ...
    const fbo = gl.createFramebuffer();
    if (!fbo) throw new Error("Failed to create composite apply FBO.");
    gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, tex, 0);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    gl.bindTexture(gl.TEXTURE_2D, this.texture);

    this.compositeApplyFbo = fbo;
    this.compositeApplyTexture = tex;
    this.compositeApplyFboWidth = width;
    this.compositeApplyFboHeight = height;
    this.compositeApplyTextureSamplingFilter = gl.NEAREST;
  }
  ...
  private ensurePass2SamplingFbo(width: number, height: number) {
    if (
      this.pass2SamplingFboWidth === width &&
      this.pass2SamplingFboHeight === height &&
      this.pass2SamplingFbo
    ) {
      return;
    }
    ...
    const tex = gl.createTexture();
    if (!tex) throw new Error("Failed to create pass2 sampling FBO texture.");
    gl.bindTexture(gl.TEXTURE_2D, tex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
    ...
    const fbo = gl.createFramebuffer();
    if (!fbo) throw new Error("Failed to create pass2 sampling FBO.");
    gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, tex, 0);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    gl.bindTexture(gl.TEXTURE_2D, this.texture);

    this.pass2SamplingFbo = fbo;
    this.pass2SamplingTexture = tex;
    this.pass2SamplingFboWidth = width;
    this.pass2SamplingFboHeight = height;
    this.pass2SamplingTextureSamplingFilter = gl.NEAREST;
  }
  ...
  private ensureFinalSceneFbo(width: number, height: number) {
    if (
      this.finalSceneFboWidth === width &&
      this.finalSceneFboHeight === height &&
      this.finalSceneFbo
    ) {
      return;
    }
    ...
    const tex = gl.createTexture();
    if (!tex) throw new Error("Failed to create final-scene FBO texture.");
    gl.bindTexture(gl.TEXTURE_2D, tex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
    ...
    const fbo = gl.createFramebuffer();
    if (!fbo) throw new Error("Failed to create final-scene FBO.");
    gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, tex, 0);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    gl.bindTexture(gl.TEXTURE_2D, this.texture);

    this.finalSceneFbo = fbo;
    this.finalSceneTexture = tex;
    this.finalSceneFboWidth = width;
    this.finalSceneFboHeight = height;
    this.finalSceneTextureSamplingFilter = gl.LINEAR;
  }

  private ensureWideGlowQuarterFbo(width: number, height: number) {
    if (
      this.wideGlowQuarterFboWidth === width &&
      this.wideGlowQuarterFboHeight === height &&
      this.wideGlowQuarterFbo
    ) {
      return;
    }
    ...
    const tex = gl.createTexture();
    if (!tex) throw new Error("Failed to create wide-glow quarter FBO texture.");
    gl.bindTexture(gl.TEXTURE_2D, tex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
    ...
    const fbo = gl.createFramebuffer();
    if (!fbo) throw new Error("Failed to create wide-glow quarter FBO.");
    gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, tex, 0);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    gl.bindTexture(gl.TEXTURE_2D, this.texture);

    this.wideGlowQuarterFbo = fbo;
    this.wideGlowQuarterTexture = tex;
    this.wideGlowQuarterFboWidth = width;
    this.wideGlowQuarterFboHeight = height;
    this.wideGlowQuarterTextureSamplingFilter = gl.LINEAR;
  }

  private ensureWideGlowBlurFbo(width: number, height: number) {
    if (
      this.wideGlowBlurFboWidth === width &&
      this.wideGlowBlurFboHeight === height &&
      this.wideGlowBlurFbo
    ) {
      return;
    }
    ...
    const tex = gl.createTexture();
    if (!tex) throw new Error("Failed to create wide-glow blur FBO texture.");
    gl.bindTexture(gl.TEXTURE_2D, tex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
    ...
    const fbo = gl.createFramebuffer();
    if (!fbo) throw new Error("Failed to create wide-glow blur FBO.");
    gl.bindFramebuffer(gl.FRAMEBUFFER, fbo);
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, tex, 0);
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    gl.bindTexture(gl.TEXTURE_2D, this.texture);

    this.wideGlowBlurFbo = fbo;
    this.wideGlowBlurTexture = tex;
    this.wideGlowBlurFboWidth = width;
    this.wideGlowBlurFboHeight = height;
    this.wideGlowBlurTextureSamplingFilter = gl.LINEAR;
  }
  ...
  private syncCompositeApplyTextureSamplingFilter(nextFilter: number) {
    if (!this.compositeApplyTexture || this.compositeApplyTextureSamplingFilter === nextFilter) {
      return;
    }

    const { gl } = this;
    gl.bindTexture(gl.TEXTURE_2D, this.compositeApplyTexture);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, nextFilter);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, nextFilter);
    this.compositeApplyTextureSamplingFilter = nextFilter;
  }
  ...
  private syncPass2SamplingTextureSamplingFilter(nextFilter: number) {
    if (!this.pass2SamplingTexture || this.pass2SamplingTextureSamplingFilter === nextFilter) {
      return;
    }

    const { gl } = this;
    gl.bindTexture(gl.TEXTURE_2D, this.pass2SamplingTexture);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, nextFilter);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, nextFilter);
    this.pass2SamplingTextureSamplingFilter = nextFilter;
  }
  ...
  private syncFinalSceneTextureSamplingFilter(nextFilter: number) {
    if (!this.finalSceneTexture || this.finalSceneTextureSamplingFilter === nextFilter) {
      return;
    }
    ...
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, nextFilter);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, nextFilter);
    this.finalSceneTextureSamplingFilter = nextFilter;
  }

  private syncWideGlowQuarterTextureSamplingFilter(nextFilter: number) {
    if (!this.wideGlowQuarterTexture || this.wideGlowQuarterTextureSamplingFilter === nextFilter) {
      return;
    }
    ...
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, nextFilter);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, nextFilter);
    this.wideGlowQuarterTextureSamplingFilter = nextFilter;
  }

  private syncWideGlowBlurTextureSamplingFilter(nextFilter: number) {
    if (!this.wideGlowBlurTexture || this.wideGlowBlurTextureSamplingFilter === nextFilter) {
      return;
    }
    ...
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, nextFilter);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, nextFilter);
    this.wideGlowBlurTextureSamplingFilter = nextFilter;
  }
  ...
  setFilterPrograms(
    ...
    this.setCompositePrepProgram(compositePrep ?? null);
  ...
  private setCompositePrepProgram(program: WebGLProgram | null) {
    this.compositePrepProgram = program;
    if (!program) {
      this.compositePrepLocs = null;
      return;
    }
  ...
  private applyCompositePrepUniforms(
    filterState: RetroVideoFilterState,
    sourceWidth?: number,
    sourceHeight?: number,
    targetWidth?: number,
    ...
    if (!this.compositePrepLocs) {
      return;
    }

    const resolvedTargetWidth =
      targetWidth ?? this.resolvePass1TargetSize(filterState, sourceWidth, sourceHeight).width;
    const resolvedTargetHeight =
      targetHeight ?? this.resolvePass1TargetSize(filterState, sourceWidth, sourceHeight).height;
    const { gl } = this;
    gl.uniform2f(this.compositePrepLocs.uTargetSize, resolvedTargetWidth, resolvedTargetHeight);
    gl.uniform1f(this.compositePrepLocs.uSamplingMode, getSamplingModeValue(filterState.samplingMode));
  }

  private applyCompositeMidUniforms(
    filterState: RetroVideoFilterState,
    ...
    const { width, height } = this.resolvePass1TargetSize(filterState, sourceWidth, sourceHeight);
    const { gl } = this;
    gl.uniform2f(this.compositeMidLocs.uTargetSize, width, height);
    ...
    return {
      ...
      compositeMid:
        pass1Variant === "basic_composite" || pass1Variant === "pc98_composite"
          ? FILTER_FRAGMENT_PASS_COMPOSITE_APPLY
          : null,
      phosphorCore:
    ...
    const compilePromise = (async () => {
      ...
      try {
        ...
        const entry: WindowsLiteCompiledPrograms = {
          pass1: pass1Program,
          pass2: pass2Program,
          ...(compositePrepProgram ? { compositePrep: compositePrepProgram } : {}),
  ...
  private async ensureWideGlowCompositeProgram() {
    ...
    try {
      ...
      this.gl.uniform1i(this.gl.getUniformLocation(program, "uGlowTexture"), 1);
  ...
  private hasPreparedSupportProgramsForFilterState(filterState: RetroVideoFilterState) {
    ...
    const needsWideGlow = shouldUseWideGlow(filterState);
    ...
    return hasBeamDownscale && hasPostCurvature && hasWideGlow;
  }
  ...
  render() {
    ...
    if (usingFilter) {
      ...
      const pass1TargetSize = this.resolvePass1TargetSize(
        filterState,
        sourceSize.width,
        sourceSize.height,
      );
      ...
      if (usePass1SamplingPrep && this.compositePrepProgram && this.compositePrepLocs) {
        ...
        gl.bindTexture(gl.TEXTURE_2D, this.texture);
        this.syncTextureSamplingFilter(textureFilter);
        this.applyCompositePrepUniforms(
          filterState,
          sourceSize.width,
          sourceSize.height,
        );
      ...
      this.ensureFbo(pass1RenderWidth, pass1RenderHeight);
      gl.bindFramebuffer(gl.FRAMEBUFFER, this.fbo);
      gl.viewport(0, 0, pass1RenderWidth, pass1RenderHeight);
      gl.clearColor(0, 0, 0, 1);
      gl.clear(gl.COLOR_BUFFER_BIT);
      ...
      if (this.compositeMidProgram && this.compositeMidLocs && this.compositeMidTexture) {
        this.ensureCompositeApplyFbo(w, h);
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.compositeApplyFbo);
        gl.viewport(0, 0, w, h);
        gl.clearColor(0, 0, 0, 1);
        gl.clear(gl.COLOR_BUFFER_BIT);
        ...
        gl.bindTexture(gl.TEXTURE_2D, this.compositeMidTexture);
        this.syncCompositeMidTextureSamplingFilter(gl.NEAREST);
        this.applyCompositeMidUniforms(
          filterState,
          sourceSize.width,
          sourceSize.height,
          timeSec,
        ...
        gl.drawArrays(gl.TRIANGLES, 0, 6);
        pass1OutputTexture = this.compositeApplyTexture;
      }
      ...
      if (canUseWideGlow) {
        this.ensureFinalSceneFbo(w, h);
      }
      const finalSceneTargetFbo = canUseWideGlow ? this.finalSceneFbo : null;
      gl.bindFramebuffer(
        gl.FRAMEBUFFER,
        canUsePostCurvaturePass ? this.postCurvatureFbo : finalSceneTargetFbo,
      );
      gl.viewport(0, 0, w, h);
      gl.clearColor(0, 0, 0, 1);
      gl.clear(gl.COLOR_BUFFER_BIT);
      ...
      const usePass2Sampling = shouldUsePass2SamplingPrep(filterState);
      if (canUsePreFilterDownscale && this.beamDownscaleProgram && this.beamDownscaleLocs) {
        ...
        if (pass1OutputTexture === this.compositeApplyTexture) {
          this.syncCompositeApplyTextureSamplingFilter(gl.LINEAR);
        } else if (pass1OutputTexture === this.compositeMidTexture) {
      ...
      if (
        usePass2Sampling &&
        this.compositePrepProgram &&
        this.compositePrepLocs &&
        pass2PrimaryTexture &&
        !isBeamVariant
      ) {
        pass2Sizing ??= this.resolvePass2Sizing(filterState, sourceSize.width, sourceSize.height);
        this.ensurePass2SamplingFbo(pass2Sizing.pass2TargetWidth, pass2Sizing.pass2TargetHeight);
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.pass2SamplingFbo);
        gl.viewport(0, 0, pass2Sizing.pass2TargetWidth, pass2Sizing.pass2TargetHeight);
        gl.clearColor(0, 0, 0, 1);
        gl.clear(gl.COLOR_BUFFER_BIT);
        gl.useProgram(this.compositePrepProgram);
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, pass2PrimaryTexture);
        if (pass2PrimaryTexture === this.compositeApplyTexture) {
          this.syncCompositeApplyTextureSamplingFilter(gl.NEAREST);
        } else if (pass2PrimaryTexture === this.compositeMidTexture) {
          this.syncCompositeMidTextureSamplingFilter(gl.NEAREST);
        } else if (pass2PrimaryTexture === this.phosphorCoreTexture) {
          this.syncPhosphorCoreTextureSamplingFilter(gl.NEAREST);
        } else if (pass2PrimaryTexture === this.beamSourceTexture) {
        ...
        this.applyCompositePrepUniforms(
          ...
          sourceSize.width,
          sourceSize.height,
          pass2Sizing.pass2TargetWidth,
          pass2Sizing.pass2TargetHeight,
        );
        gl.drawArrays(gl.TRIANGLES, 0, 6);
        gl.bindFramebuffer(
          gl.FRAMEBUFFER,
        ...
        gl.viewport(0, 0, w, h);
        pass2PrimaryTexture = this.pass2SamplingTexture ?? pass2PrimaryTexture;
      }
      if (this.phosphorCoreProgram && this.phosphorCoreLocs && pass2PrimaryTexture) {
        ...
        if (pass2PrimaryTexture === this.beamSourceTexture) {
          this.syncBeamSourceTextureSamplingFilter(gl.NEAREST);
        } else if (pass2PrimaryTexture === this.compositeApplyTexture) {
          this.syncCompositeApplyTextureSamplingFilter(gl.NEAREST);
        } else if (pass2PrimaryTexture === this.compositeMidTexture) {
          this.syncCompositeMidTextureSamplingFilter(gl.NEAREST);
        } else if (pass2PrimaryTexture === this.pass2SamplingTexture) {
          this.syncPass2SamplingTextureSamplingFilter(gl.NEAREST);
        } else {
          this.syncFboTextureSamplingFilter(gl.NEAREST);
        }
      ...
      if (pass2PrimaryTexture === this.beamSourceTexture) {
        ...
        this.syncCompositeApplyTextureSamplingFilter(pass2TextureFilter);
      } else if (pass2PrimaryTexture === this.phosphorCoreTexture) {
        ...
        this.syncPass2SamplingTextureSamplingFilter(pass2TextureFilter);
      } else if (pass2PrimaryTexture === this.compositeMidTexture) {
      ...
      if (canUsePostCurvaturePass && this.postCurvatureTexture && this.postCurvatureProgram && this.postCurvatureLocs) {
        gl.bindFramebuffer(gl.FRAMEBUFFER, finalSceneTargetFbo);
        gl.viewport(0, 0, w, h);
        gl.clearColor(0, 0, 0, 1);
        gl.clear(gl.COLOR_BUFFER_BIT);
      ...
      if (
        ...
        const wideGlowUpdateInterval = isOpticalWideGlow
          ? getWideGlowUpdateInterval(filterState)
          : 1;
        const wideGlowCacheKey = [
          isOpticalWideGlow ? "optical" : "smoky",
          glowDivisor,
          glowWidth,
          glowHeight,
          w,
          ...
          blurRadiusSecondary.toFixed(3),
        ].join(":");
        const shouldRefreshWideGlow =
          !this.wideGlowCacheInitialized ||
          this.wideGlowCacheKey !== wideGlowCacheKey ||
          wideGlowUpdateInterval <= 1 ||
          this.renderCount % wideGlowUpdateInterval === 0;
        this.ensureWideGlowQuarterFbo(glowWidth, glowHeight);
        this.ensureWideGlowBlurFbo(glowWidth, glowHeight);

        if (shouldRefreshWideGlow && wideGlowDownsampleProgram && wideGlowDownsampleLocs) {
        ...
        if (shouldRefreshWideGlow) {
          gl.bindFramebuffer(gl.FRAMEBUFFER, this.wideGlowBlurFbo);
          gl.viewport(0, 0, glowWidth, glowHeight);
          gl.useProgram(this.wideGlowBlurProgram);
          gl.activeTexture(gl.TEXTURE0);
          gl.bindTexture(gl.TEXTURE_2D, this.wideGlowQuarterTexture);
          ...
          this.wideGlowCacheInitialized = true;
          this.wideGlowCacheKey = wideGlowCacheKey;
        }
        ...
        gl.clearColor(0, 0, 0, 1);
        gl.clear(gl.COLOR_BUFFER_BIT);
        gl.useProgram(this.wideGlowCompositeProgram);
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, this.finalSceneTexture);
        this.syncFinalSceneTextureSamplingFilter(gl.LINEAR);
        gl.activeTexture(gl.TEXTURE1);
  ...
  dispose() {
    ...
    this.lastUploadedVideoSource = null;
    this.lastUploadedVideoTime = Number.NaN;
    this.wideGlowCacheInitialized = false;
    this.wideGlowCacheKey = "";
  }
  ...
  private resolvePass1TargetSize(
    filterState: RetroVideoFilterState,
    sourceWidth?: number,
    sourceHeight?: number,
  ) {
  ...
  private applyPass1Uniforms(
    ...
    const { gl } = this;
    const { width: effectiveTargetWidth, height: effectiveTargetHeight } =
      this.resolvePass1TargetSize(filterState, sourceWidth, sourceHeight);
  ...
  private applyPass2UniformsTo(
    ...
    gl.uniform1f(
      locs.uSamplingMode,
      shouldUsePass2SamplingPrep(filterState) ? 0 : getSamplingModeValue(filterState.samplingMode),
    );
