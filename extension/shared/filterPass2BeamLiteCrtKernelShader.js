// Generated by scripts/sync-extension-shader.mjs from src/retro-player/retro/filterPass2BeamLiteCrtKernelShader.ts
export const FILTER_FRAGMENT_PASS2_BEAM_LITE_CRT_KERNEL = `#version 300 es
...
float getBeamDarkCutoff() { return clamp(0.0400, 0.0, 0.15); }
float getBeamHorizontalSpread() { return clamp(1.2700, 0.5, 2.0); }
float getBeamWhiteBloom() { return clamp(1.1400, 0.0, 2.0); }
...
vec3 sampleEmitterColor(vec2 emitterCell, vec2 sourceSize) {
  ...
  if (0.0000 < 1.5) return sampleSourceTextureAverage4(cellMin, cellSize);
  if (0.0000 < 2.5) return sampleSourceTextureAverage8(cellMin, cellSize);
  return sampleSourceTextureAverage8(cellMin, cellSize);
}

vec3 sampleEmitterColorConverged(vec2 emitterCell, vec2 sourceSize) {
  float convergenceOffset = max(0.0000, 0.0);
...
vec3 applyBeamInputHorizontalSharpness(vec3 center, vec3 left, vec3 right) {
  float amount = clamp(1.0000 - 1.0, -1.0, 1.0);
...
vec3 applyBeamInputPostProcess(vec3 center, vec3 left, vec3 right, vec3 up, vec3 down, vec2 emitterCell) {
  ...
  if (0.0000 > 0.001) {
    ...
    color = mix(color, blurred, clamp(0.0000, 0.0, 1.0));
  }
...
vec3 applyBeamCross(vec2 gridUv) {
  ...
  bool needsHorizontalNeighbors = abs(1.0000 - 1.0) > 0.0001 || 0.0000 > 0.001;
  bool needsVerticalNeighbors = 0.0000 > 0.001;
...
void main(void) {
  vec2 curvedUv = curveUv(vTextureCoord, 0.0300);
