...
If you are only iterating on the extension, you do not need `npm run dev`.

```bash
npm run prepare:extension
```

If you want shader edits from `src/retro-player/retro/filterShader.ts` to keep syncing into the unpacked extension while you work:
...
6. Reload the captured tab as well when you change the content script or capture flow
7. Open a tab you want to capture
8. Click the extension button
9. In the popup, press `Capture current tab`
...
sh deploy_mac.sh
% ~/bin/butler login
% ~/bin/butler push src-tauri/target/aarch64-apple-darwin/release/bundle/dmg/tetorica-retro-player_0.9.1_aarch64.dmg kyorohiro/tetorica-retro-player:mac-apple-silicon --userversion 0.9.1

% ~/bin/butler push src-tauri/target/x86_64-apple-darwin/release/bundle/dmg/tetorica-retro-player_0.9.1_x64.dmg kyorohiro/tetorica-retro-player:mac-intel --userversion 0.9.1

% ~/bin/butler push "tetorica-retro-player_0.9.1_x64-setup.exe" kyorohiro/tetorica-retro-player:windows --userversion 0.9.1

% ~/bin/butler push "tetorica-retro-player_0.9.1_aarch64.AppImage" kyorohiro/tetorica-retro-player:linux-arm --userversion 0.9.1
% ~/bin/butler push "tetorica-retro-player_0.9.1_amd64.AppImage" kyorohiro/tetorica-retro-player:linux-intel --userversion 0.9.1

~/bin/butler push \
  "app-release-signed_0.9.1.apk" \
...
~/Library/Android/sdk/build-tools/35.0.0/apksigner sign \
  ...
  --out app-release-signed_0.9.1.apk \
