...
```sh
npm run version:set -- 0.21.10
```

```
sh deploy_mac.sh
% ~/bin/butler login
% ~/bin/butler push src-tauri/target/aarch64-apple-darwin/release/bundle/dmg/tetorica-retro-player_0.21.10_aarch64.dmg kyorohiro/tetorica-retro-player:mac-apple-silicon --userversion 0.21.10

% ~/bin/butler push src-tauri/target/x86_64-apple-darwin/release/bundle/dmg/tetorica-retro-player_0.21.10_x64.dmg kyorohiro/tetorica-retro-player:mac-intel --userversion 0.21.10

% ~/bin/butler push "tetorica-retro-player_0.21.10_x64-setup.exe" kyorohiro/tetorica-retro-player:windows --userversion 0.21.10

% ~/bin/butler push "tetorica-retro-player_0.21.10_aarch64.AppImage" kyorohiro/tetorica-retro-player:linux-arm --userversion 0.21.10

% ~/bin/butler push "tetorica-retro-player_0.21.10_amd64.AppImage" kyorohiro/tetorica-retro-player:linux-intel --userversion 0.21.10

~/bin/butler push \
  "app-release-signed_0.21.10.apk" \
...
npm run build
cd dist
zip -r ../web-build_0.21.10_gh.zip .
```
...
~/Library/Android/sdk/build-tools/35.0.0/apksigner sign \
  ...
  --out app-release-signed_0.21.10.apk \
